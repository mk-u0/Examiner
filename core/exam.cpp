...
Result::Result(int exam_id, int student_id, double grade) {
...
int Exam::findIndexByID(int id) const {
...
const Question &Exam::getQuestion(int i) const {
    return questions[i];
}

const Question &Exam::getQuestionByID(int id) const {
...
void Exam::removeQuestion(int i) {
    questions.erase(questions.begin() + i);
}

void Exam::removeQuestionByID(int id) {
    questions.erase(questions.begin() + findIndexByID(id));
}

void Exam::modifyQuestion(int i, const Question &q) {
    questions[i] = q;
}

void Exam::modifyQuestionByID(int id, const Question &q) {
    questions[findIndexByID(id)] = q;
}


std::vector<bool> Exam::checkAnswer(const std::vector<int> &answer) {
...
Result Exam::getResult(const std::vector<int> &answer, int student_id) {
    std::vector<bool> res = checkAnswer(answer);
    int correct_num = 0;
    for (unsigned i = 0; i < size(); i++) {
        correct_num += static_cast<int>(res[i]);
    }
